...
## 實測（2026-08-28，`.profile` 卡片化與 about 重排之後）

| 路由 | Perf | A11y | BP | SEO | LCP | CLS | TBT |
|---|---|---|---|---|---|---|---|
| `/` | 98 | 100 | 93 | 100 | 1212 ms | 0.000 | 178 ms |
| `/about/` | 98 | 100 | 96 | 100 | 1206 ms | 0.000 | 170 ms |
| `/writing/` | 98 | 100 | 96 | 100 | 1206 ms | 0.000 | 165 ms |
| `/writing/approval-orchestrator/` | 100 | 100 | 96 | 100 | 1206 ms | 0.000 | 0 ms |
| `/zh/` | 96 | 100 | 93 | 100 | 1206 ms | 0.000 | 218 ms |
| `/zh/writing/aks-lun-exhaustion/` | 100 | 100 | 96 | 100 | 1206 ms | 0.000 | 0 ms |

`.profile` 從三行散文改成五個欄位、about 重排成六個小標之後重測，六個 URL 的四個類別中位數皆 ≥ 0.90，`npm run test:lighthouse` 第一次以正確設定執行就全部通過，未使用補救 A/B/C（詳見下方「執行紀錄」）。

## 執行紀錄

- `/`、`/zh/` 載入 three.js 播放入侵序列，但 LCP（1212 ms / 1206 ms）與 TBT（178 ms / 218 ms）皆遠低於 2500 ms / 300 ms 門檻，未觸發補救 A（延遲載入 three.js）或補救 B（檢查 `seq-pending`）。
